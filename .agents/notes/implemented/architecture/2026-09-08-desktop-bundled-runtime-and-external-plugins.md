...
[Runtime preparation](../../../../apps/desktop/scripts/prepare-dsh.ts) materializes the production graph once at build time and ships it as one verified archive that the first launch expands into the Harness home ([runtime-archive decision](2026-09-13-desktop-runtime-archive.md)). The Electron shell stays in ASAR. A bundled upstream Node process runs the private Desktop Host from the expanded tree and loads enabled plugins from `$DSH_HOME/profiles/desktop`.
...
The resource descriptor inside the archive records the exact release, Node version, platform, architecture, shared package versions, and final file hashes. The runtime tree contains ordinary files and directories, without links back to pnpm’s build store. Native Mach-O files are signed before hashing; the packer preserves their bytes, and the [runtime-archive decision](2026-09-13-desktop-runtime-archive.md) owns how the tree is packed, verified, and expanded.
...
Full per-file runtime verification belongs to packaging. Startup reads the descriptor, checks shared package records and required Host entries, and uses the recorded runtime identity for profile reuse. The [release-validation decision](2026-09-09-desktop-build-release-validation.md) assigns release and target compatibility checks to packaging. Reading every file before backend loading adds startup I/O proportional to the distribution size, so startup compares the expanded tree against one recorded digest instead ([runtime-archive decision](2026-09-13-desktop-runtime-archive.md)), and unusable modules still fail when they load. Build-time verification rejects changed, missing, extra, or linked files against the recorded inventory.
