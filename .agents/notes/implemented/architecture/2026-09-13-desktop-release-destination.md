...
- `electron-builder.config.mjs` publishes with `provider: 'github'` and the repository coordinates, so the packaged `app-update.yml` names the repository and the updater resolves the newest release from it. `releaseType` follows the version: a prerelease version publishes a GitHub prerelease.
...
The macOS lanes run on hosted runners. Both need the signing certificate and the notarization key as secrets in the `desktop-release` environment, and the x64 lane needs an Intel runner or an Apple Silicon runner with Rosetta, so the workflow names `macos-13` for it.
