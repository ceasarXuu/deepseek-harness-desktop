...
本地加载页报告启动状态，并在失败时说明失败原因和每个恢复操作的作用；失败文本会保留在下方操作按钮可见期间。重启始终提供，它会关闭应用并重新启动。禁用第三方插件和重置 Desktop 仅在已初始化应用能够修复其 profile 时提供，而这依赖已打包的应用资源，因此开发项目和早期初始化失败只提供重启。两个会修改 profile 的操作以危险样式呈现，并各自带有一条说明：禁用会保留插件文件并在不加载它们的情况下启动；重置会删除桌面端全部配置和第三方插件且不保留备份，然后启动全新的 profile。点击某个操作后会保持忙碌状态，禁用全部操作并说明正在执行；若该操作失败，页面会报告这次失败，而不会保留原来的文本。当桌面壳资源或 preload 失败导致加载页本身无法加载时，一个独立文档会携带相同的失败文本，始终提供重启，且仅在具备 profile 恢复能力时才提供这两个 profile 操作。

### 首次启动

未找到 profile 而启动的已安装应用会在工作区打开后显示一次首次启动指引。原生对话框说明本应用运行在自带的运行时之上、指出该运行时的位置，并指明从应用菜单打开的“桌面插件”窗口是管理插件的位置。它的唯一操作会继续进入应用，其“不再显示”复选框会写入 `$DSH_HOME/desktop/first-launch-guide.json`。该选择保存在 profile 旁边而非其中的 Electron 自有状态里，因此能挺过重置；启动时已存在 profile 的，或任何开发项目，都不会显示该指引。

### 诊断信息

“桌面插件”窗口有一个“诊断信息”区块，列出应用版本、dsh 运行时版本、Harness 主目录、运行时位置，以及上次启动的结果——无论是成功，还是失败及其消息。主进程把这些事实连同本地化标签组装成一个文本块；复制操作会把该文本块放入剪贴板，显示操作会在文件管理器中打开 Harness 主目录。渲染进程通过桌面桥接只接收这一个组装好的载荷，并逐条事实渲染一行，因此它自身不读取文件系统、也不调用 shell；它暂时无法读取的版本（例如运行时描述文件尚不存在时）会显示为“不可用”，而不是让该区块失败。
...
一次检查会把它的阶段发布到每个窗口，并保留可用版本。原生确认框会指明该版本，并说明此版本包含配套的 dsh 运行时、应用将在安装完成后重新启动，因此用户在确认前就能看到该版本和这次重启。拒绝会让该版本继续保持已知，因此之后无法访问更新源的检查会报告错误，同时让被拒绝的版本仍可安装。接受会等待正在进行的检查，然后下载并验证该版本；下载失败会得到报告，并让该版本仍可重试。下载成功后，应用会停止 dsh 子进程并把安装与重启交给 electron-updater，交接失败同样会得到报告：应用继续以旧版本运行，而更新仍可安装。
