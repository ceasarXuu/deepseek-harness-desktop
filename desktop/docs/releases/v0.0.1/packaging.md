...
A build leaves only what a release publishes: the DMG, the ZIP the updater consumes, their block maps, and `latest-mac.yml`. electron-builder stages the assembled application under `dist/mac-<arch>` and produces both artifacts from it; that staging is roughly half the output size and exactly reproducible, so `package-app.mjs` removes it once the artifacts exist. A build that cannot find its artifacts keeps the staging instead, because that is the state worth diagnosing.

The closure archive under `build/out` is a build input rather than an output, and it is regenerated by `build-closure.mjs` when it is absent. Nothing else persists — test installations, spike directories, and superseded artifacts are removed rather than accumulated.
