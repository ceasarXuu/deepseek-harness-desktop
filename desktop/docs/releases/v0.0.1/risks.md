...
| A DMG is distributed before notarization credentials exist | High until they are configured | Medium — the recipient cannot launch the application | Signing alone produces an artifact that works only where it is accepted explicitly. The distinction is stated in [`packaging.md`](packaging.md) so a signed build is not mistaken for a distributable one |
